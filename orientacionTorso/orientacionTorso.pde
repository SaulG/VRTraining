...
void draw() {
  ...
  for(int i = 0; i < listaUsuarios.length; i++){
    ...
    if(context.isTrackingSkeleton(listaUsuarios[i])){
       ...
       orientacionTorso(listaUsuarios[i]);
     }        
...
void orientacionTorso(int usuarioId){
 ...
 if(grados < 70.0){
    text(String.format("Grados: %.2f Orientacion: derecha", grados), 10, 80);;
 }else if(grados > 110.0){
    text(String.format("Grados: %.2f Orientacion: izquierda", grados), 10, 80);
 }else{
    text(String.format("Grados: %.2f Orientacion: neutral", grados), 10, 80);
 }
}
